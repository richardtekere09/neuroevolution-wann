...
def run_evolution(
    ...
    """
    Run the evolutionary algorithm to evolve WANNs.

    Args:
        generations: Number of generations to run
        population_size: Size of the population
        weight_candidates: List of weight values to try during evaluation
    ...
    for gen in range(generations):
        ...
        for net in population:
            best_reward, best_weight = evaluate_network(
                net, env, weight_candidates, render=False
            )
            fitness_scores.append(best_reward)
        ...
        # Save the best network from this generation
        save_network(population[0], gen)

        # Plot the fitness history
        plot_fitness(fitness_history, filename="plots/fitness_basic.png")
...
def get_top_generations(
    ...
    """
    Evaluate all saved networks and return the top performing ones.

    Args:
        folder: Folder containing the saved networks
        shared_weights: List of weight values to try during evaluation
        top_n: Number of top networks to return

    Returns:
        List of tuples (generation, fitness) for the top networks
    """
    ...
    for file in os.listdir(folder):
        if file.startswith("generation_") and file.endswith(".json"):
            ...
            fitness, _ = evaluate_network(net, env, shared_weights)
